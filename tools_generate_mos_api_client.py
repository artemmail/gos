...
swagger_path = pathlib.Path('mos_sw.json')
out_path = pathlib.Path('src/Zakupki.MosApi/MosSwaggerClient.cs')
...
def render_operations() -> str:
    op_lines = ["    public partial class MosSwaggerClient", "    {", "        private readonly HttpClient _httpClient;", "        private readonly JsonSerializerOptions _serializerOptions;", "        private readonly string _baseUrl;", "", "        public MosSwaggerClient(HttpClient httpClient, string baseUrl)", "        {", "            _httpClient = httpClient;", "            _baseUrl = baseUrl.TrimEnd('/');", "            _serializerOptions = new JsonSerializerOptions", "            {", "                PropertyNamingPolicy = JsonNamingPolicy.CamelCase,", "                PropertyNameCaseInsensitive = true", "            };", "        }", ""]
    for path, methods in swagger.get('paths', {}).items():
        for http_method, operation in methods.items():
            ...
            if response_type == 'void':
                ...
                return_type = f"Task<{response_type}?>"
